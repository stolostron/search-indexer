...
#  2. Update state.
    ...
    def send_full_state_payload(self):
        with open("sno-template.json", "r") as template_file:
            template_string = template_file.read().replace("<<CLUSTER_NAME>>", "anx-man")
        f = io.StringIO(template_string)
        j = json.load(f)        
        self.client.payload = j
        self.do_post()
        print("%s - sent full state" % "anx-man")

    def send_update_payload(self):
        f = open("cluster-update-template.json",)
        j = json.load(f)        
        for resource in j["addResources"]: 
            resource["uid"] = "{}/{}".format("anx-man", str(uuid.uuid4()) )
            resource["properties"]["name"] = "gen-name-{}".format(str(uuid.uuid4()) )
        self.client.payload = j
        self.do_post()
        print("%s - sent update" % "anx-man")

    def do_post(self):
        self.client.post("/aggregator/clusters/{}/sync".format("anx-man"), json=self.client.payload, verify=False)
