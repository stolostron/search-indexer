...
### `search_indexer_resources_processed_total` · CounterVec

Number of individual resource operations successfully committed to PostgreSQL, broken down by operation type. This is the primary signal for understanding the write throughput of the indexer.

| Attribute | Value |
|---|---|
| Type | `counter` |
| Labels | `operation` — `"add"`, `"update"`, `"delete"` |
...
|---|---|---|---|
| **Delta sync** (`X-Overwrite-State: false`) | Resources in `addResources[]` that were processed without error | Resources in `updateResources[]` that were processed without error | Resources in `deleteResources[]` that were processed without error |
| **Full resync** (`X-Overwrite-State: true`) | All incoming resources (upserted with `ON CONFLICT DO UPDATE`) | _(never incremented; resyncs use upsert semantics)_ | Resources found in the DB for the cluster that are absent from the incoming set |
...
# All operations per minute (combined)
sum(rate(search_indexer_resources_processed_total[1m])) * 60

# Operations per minute, split by type
sum by (operation) (rate(search_indexer_resources_processed_total[1m])) * 60

# Adds per minute
rate(search_indexer_resources_processed_total{operation="add"}[1m]) * 60

# Updates per minute
rate(search_indexer_resources_processed_total{operation="update"}[1m]) * 60

# Deletes per minute
rate(search_indexer_resources_processed_total{operation="delete"}[1m]) * 60


# --- Throughput over longer windows ---

# Total resources processed in the last hour
sum(increase(search_indexer_resources_processed_total[1h]))

# Breakdown by operation in the last 24 hours
sum by (operation) (increase(search_indexer_resources_processed_total[24h]))

# Ratio of deletes to total operations (churn indicator)
sum(rate(search_indexer_resources_processed_total{operation="delete"}[5m]))
  / sum(rate(search_indexer_resources_processed_total[5m]))


# --- Alerting ---

# Alert: no resources processed in 10 minutes (collector may be down)
sum(increase(search_indexer_resources_processed_total[10m])) == 0

# Alert: delete rate spikes above 500/min (unexpected mass deletion)
sum(rate(search_indexer_resources_processed_total{operation="delete"}[2m])) * 60 > 500

# Alert: add rate drops to zero while the indexer is still receiving requests
#   (requests coming in but nothing being inserted — possible DB issue)
(
  sum(rate(search_indexer_request_count[5m])) > 0
) and (
  sum(rate(search_indexer_resources_processed_total{operation="add"}[5m])) == 0
)
```
...
# Resources processed per second (all ops)
sum(rate(search_indexer_resources_processed_total[5m]))

# Resources processed per request (average batch size per sync)
sum(rate(search_indexer_resources_processed_total[5m]))
  / sum(rate(search_indexer_request_count[5m]))
