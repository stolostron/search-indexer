...
func Test_initializeTables(t *testing.T) {
	...
	mockPool.EXPECT().Exec(gomock.Any(), gomock.Eq("CREATE INDEX IF NOT EXISTS data_users_idx ON search.resources USING GIN ((data -> 'users')) WHERE (data -> 'kind') ? 'Group' AND (data -> 'apigroup') ? 'user.openshift.io'")).Return(nil, nil)
