		...
		for existingRows.Next() {
			...
			if !reflect.DeepEqual(incomingResource.Properties, props) {
				...
				query, params, err := useGoqu(
					...
					[]interface{}{incomingResource.UID, clusterName, string(dataByte)})
				if err == nil {
					queueErr := batch.Queue(batchItem{
						action: "addResource",
						query:  query,
